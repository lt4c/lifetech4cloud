...
| `user` | Baseline authenticated state | `user:read` |
...
| `sys:status:read` | View system + dependency health | Covers `/status/health` and `/status/deps` |
...
- `role_permissions` — mapping table linking roles ? permissions
- `user_roles` — mapping table linking users ? roles
...
Every mutating API endpoint calls `record_audit`, producing a diff of before/after state. Diff payloads are persisted in `audit_logs.diff_json` as JSONB. UI surfaces the audit feed per user/role.
